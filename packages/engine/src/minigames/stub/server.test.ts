...
describe("stubGame", () => {
  it("initializes a zero count per snapshot player", () => {
    expect(stubGame.init(snapshot, "seed", 0)).toEqual({
      counts: { p1: 0, p2: 0, p3: 0 },
    });
  });

  it("increments the acting player's count", () => {
    const s0 = stubGame.init(snapshot, "seed", 0);
  ...
  it("never self-finishes (timer-bounded only)", () => {
    expect(stubGame.isFinished(stubGame.init(snapshot, "s", 0), 999999)).toBe(
      false,
    );
  });

  it("reports counts as per-player raw scores", () => {
    let s = stubGame.init(snapshot, "seed", 0);
